...
def measure_all(dataset, X_train, vector_count, ef_construction, M, ef_values, num_queries, k):
    ...
    for ef_runtime in ef_values:
        ...
        for i, target_vector, ground_truth in zip(range(num_queries), X_test[:num_queries], results[:num_queries]):
            ...
            # logging.info(ground_truth, result)
...
def run_benchmark(dataset_name, ef_construction, M, ef_values, k=1):
    ...
    for vector_count in [800000, 900000, 1000000]:
        ...
        measure_all(dataset, X_train, vector_count, ef_construction=ef_construction, M=M, ef_values=ef_values,
                    num_queries=5, k=20)
...
def run():
    ...
    for _id, vector in zip(ids, data):
        _ids, dists = index.knn_search(vector=vector, k=11, parameters=json.dumps({"hnsw": {
            "ef_runtime": 100
        }}))
        if _id in _ids:
            correct += 1
    logging.info(f"float32 recall: {correct / len(ids)}")
