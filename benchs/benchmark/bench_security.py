...
def run():
    ...
    bucket.get_object_to_file("xiangyu/security-1m.h5", '/tmp/security-1m.h5')

    with h5py.File("/tmp/security-1m.h5", 'r') as file:
        base = np.array(file['vector_256'])
        index = vsag.Index(base.shape[1], base.shape[0], "l2", "float32")
