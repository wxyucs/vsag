...
public:
    ...
    HierarchicalNSW(SpaceInterface* s,
        ...
        if (use_reversed_edges_) {
            reversed_level0_link_list_ = (std::unordered_set<tableint>**)vsag::allocate(
                max_elements_ * sizeof(std::unordered_set<tableint>*));
            memset(reversed_level0_link_list_,
                   0,
                   max_elements_ * sizeof(std::unordered_set<tableint>*));
            reversed_link_lists_ = (std::map<int, std::unordered_set<tableint>>**)vsag::allocate(
    ...
    resizeIndex(size_t new_max_elements) override {
        ...
        auto element_levels_new =
            (int*)vsag::reallocate(element_levels_, new_max_elements * sizeof(int));
        ...
        if (use_reversed_edges_) {
            auto reversed_level0_link_list_new = (std::unordered_set<tableint>**)vsag::reallocate(
            ...
            auto reversed_link_lists_new =
                (std::map<int, std::unordered_set<tableint>>**)vsag::reallocate(
        ...
        char** linkLists_new =
            (char**)vsag::reallocate(link_lists_, sizeof(void*) * new_max_elements);
    ...
    */
    ...
    addPoint(const void* data_point, labeltype label, int level) {
        ...
        if (curlevel) {
            auto new_link_lists =
                (char*)vsag::reallocate(link_lists_[cur_c], size_links_per_element_ * curlevel + 1);
